...
// Core matching rule: numbers match if they're equal OR sum to 10
export const isValidPair = (a: number, b: number): boolean => {
  return a === b || a + b === 10;
};

// Creates a new grid based on level settings
export const generateGrid = (config: LevelConfig): Grid => {
  ...
  // Make sure players have some moves to start with
...
// Guarantees at least a few valid pairs so the game isn't impossible
const ensureSolvablePairs = (cells: GridCell[][], config: LevelConfig): void => {
  ...
  // Create some guaranteed matches by placing complementary numbers next to each other
  for (let row = 0; row < startFilledRows && pairsCreated < targetPairs; row++) {
    for (let col = 0; col < gridCols - 1 && pairsCreated < targetPairs; col += 2) {
      ...
      cells[row][col + 1].value = 10 - num; // these will sum to 10
      pairsCreated++;
    }
  }
};

// Adds a new row when player clicks the button
export const addNewRow = (grid: Grid, config: LevelConfig): Grid => {
  ...
  if (targetRow === -1) return grid; // grid is full somehow
  ...
  // Try to include at least one solvable pair in the new row
  const newRow = newCells[targetRow];
  if (newRow.length >= 2) {
    const num = Math.floor(Math.random() * (maxNum - minNum + 1)) + minNum;
    newRow[0].value = num;
    newRow[1].value = Math.random() > 0.5 ? num : 10 - num; // either duplicate or complement
  }
...
// Finds all possible valid matches in the current grid - useful for checking if game is winnable
export const findValidPairs = (grid: Grid): Pair[] => {
  const pairs: Pair[] = [];
  const activeCells = grid.cells.flat().filter(cell => cell.value > 0 && !cell.isDulled);

  // Check every combination of active cells
...
// Marks the matched cells as "dulled" - they stay visible but become inactive
...
// Quick check to see if the player can still make progress
export const canStillWin = (grid: Grid, addRowsLeft: number): boolean => {
  ...
  // Still have hope if we can add more rows
  if (addRowsLeft > 0) return true;

  return false;
};

// Win condition: all numbered cells have been matched
export const isLevelComplete = (grid: Grid): boolean => {
  const activeCells = grid.cells.flat().filter(cell => cell.value > 0 && !cell.isDulled);
  return activeCells.length === 0;
};

// Helper to count how many rows actually have numbers
export const getFilledRowsCount = (grid: Grid): number => {
  ...
  for (const row of grid.cells) {
    if (row.some(cell => cell.value > 0)) {
      ...
      break; // empty row means we're done
    }
  }
  return count;
};